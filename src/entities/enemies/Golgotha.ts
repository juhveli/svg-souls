import { Entity } from '../Entity';
import { Player } from '../Player';
import { AudioController } from '../../engine/AudioController';
import { Game } from '../../engine/Game';
import { EventManager } from '../../engine/EventManager';
import { UIManager } from '../../ui/UIManager';
import { ShardEntity } from '../ShardEntity';

const BOSS_SVG = `
    <g class="boss-body">
        <!-- Junk Ball Body -->
        <circle cx="0" cy="-30" r="40" fill="#321" stroke="#543" stroke-width="4" />
        <path d="M-30,-20 L30,-40 M-20,-50 L20,-10" stroke="#111" stroke-width="3" />
        <!-- Porcelain Mask Face -->
        <ellipse cx="0" cy="-30" rx="15" ry="20" fill="#eee" />
        <circle cx="-5" cy="-30" r="2" fill="#000" />
        <circle cx="5" cy="-30" r="2" fill="#000" />
        <path d="M-5,-20 Q0,-15 5,-20" stroke="#000" stroke-width="1" fill="none" />
        <!-- Arms -->
        <line x1="-40" y1="-30" x2="-60" y2="-10" stroke="#432" stroke-width="8" />
        <line x1="40" y1="-30" x2="60" y2="-10" stroke="#432" stroke-width="8" />
    </g>
`;

export class Golgotha extends Entity {
    target: Player;
    hp: number = 200; // Tanky
    maxHp: number = 200;
    state: 'IDLE' | 'CHASE' | 'ATTACK' | 'PHASE2' = 'IDLE';

    private beatListener: () => void;
    private attackListener: EventListener;

    constructor(x: number, y: number, target: Player) {
        super(x, y, BOSS_SVG);
        this.target = target;
        this.radius = 40; // Bigger hitbox

        // Subscribe to Beat
        this.beatListener = () => this.onBeat();
        AudioController.getInstance().subscribeToBeat(this.beatListener);

        // Attack Listener
        this.attackListener = ((e: CustomEvent) => {
            if (this.isDead) return;
            const atk = e.detail;
            const dx = this.x - atk.x;
            const dy = this.y - atk.y;
            const dist = Math.sqrt(dx * dx + dy * dy);
            if (dist < atk.range + this.radius) {
                this.takeDamage(10);
            }
        }) as EventListener;
        document.addEventListener('player-attack', this.attackListener);
    }

    destroy() {
        document.removeEventListener('player-attack', this.attackListener);
        AudioController.getInstance().unsubscribeFromBeat(this.beatListener);
        super.destroy();
    }

    takeDamage(amount: number) {
        this.hp -= amount;

        // Visual Juice
        const game = Game.getInstance();
        if (game && game.particles) {
            game.particles.emit(this.x, this.y, '#fff', 10); // Porcelain dust
        }

        // Flash Red
        this.el.style.filter = 'drop-shadow(0 0 10px red)';
        setTimeout(() => this.el.style.filter = 'none', 100);

        // Phase 2 Trigger
        if (this.hp <= 100 && this.state !== 'PHASE2') {
            this.state = 'PHASE2';
            UIManager.getInstance().showBark(this.x, this.y, "THE SILENCE IS DEAFENING!");
            // Pushback
            this.x += (Math.random() - 0.5) * 100;
            this.y += (Math.random() - 0.5) * 100;
        }

        if (this.hp <= 0) {
            UIManager.getInstance().showBark(this.x, this.y, "Finally... I can sleep.");
            EventManager.getInstance().emit('ENTITY_DIED', { type: 'golgotha', id: this.id, x: this.x, y: this.y });
            game.particles.emit(this.x, this.y, '#000', 50);

            // SPAWN BOSS SHARDS (Massive reward)
            const shardCount = 15 + Math.floor(Math.random() * 11); // 15-25 shards
            for (let i = 0; i < shardCount; i++) {
                const shard = new ShardEntity(
                    this.x + (Math.random() - 0.5) * 60,
                    this.y + (Math.random() - 0.5) * 60,
                    10 + Math.floor(Math.random() * 10), // 10-19 value each
                    this.target
                );
                game.entityManager.add(shard);
            }

            this.destroy();
        }
    }

    onBeat() {
        if (this.state === 'IDLE' || this.state === 'CHASE') {
            // Move towards player
            const dx = this.target.x - this.x;
            const dy = this.target.y - this.y;
            const dist = Math.sqrt(dx * dx + dy * dy);

            if (dist < 100) {
                this.state = 'ATTACK';
                UIManager.getInstance().showBark(this.x, this.y, "CRUSH.");
            } else {
                this.x += dx * 0.05;
                this.y += dy * 0.05;
            }
        } else if (this.state === 'ATTACK') {
            // Lunge
            const dx = this.target.x - this.x;
            const dy = this.target.y - this.y;
            this.x += dx * 0.3;
            this.y += dy * 0.3;
            this.target.takeDamage(20);
            this.state = 'IDLE'; // Cooldown
        } else if (this.state === 'PHASE2') {
            // Fast Chase
            const dx = this.target.x - this.x;
            const dy = this.target.y - this.y;
            this.x += dx * 0.1;
            this.y += dy * 0.1;
            if (Math.random() < 0.3) {
                this.state = 'ATTACK';
            }
        }
    }

    update(dt: number) {
        super.update(dt);
        this.render();
    }
}
